...
SAFETY_RULES: List[Tuple[str, str, str]] = [
    (
        "gas_hazard",
        r"\b(gas leak|gas line|gas smell|smell.*gas|propane|methane|gas ignition|pilot light|gas valve|odor of gas)\b",
        "DANGER: Potential gas leak or gas appliance ignition issue. Gas repairs carry high risks of explosion, fire, or carbon monoxide poisoning and must be handled by a licensed professional gas technician."
    ),
    (
        "electrical_hazard",
        r"\b(exposed wire|frayed wire|frayed cord|sparking|burning smell|smoke from wire|short circuit|arc flash|live wire|shock hazard|scorched wire)\b",
        "DANGER: Electrical hazard detected (exposed wiring, sparking, or burning). Attempting DIY work presents severe risk of electrical shock or house fire."
    ),
    (
        "breaker_retrip",
...
def evaluate_safety(diagnosis: ApplianceDiagnosis) -> ApplianceDiagnosis:
    ...
    if diagnosis.confidence_score < 0.5:
        reasons.append(
            ...
            "Because the appliance or issue cannot be identified with high confidence, DIY repair is unsafe."
        )

    # Rule Check 2-6: Keyword / Hazard Regex Matching
    for rule_id, pattern, warning_msg in SAFETY_RULES:
        if re.search(pattern, text_corpus, re.IGNORECASE):
            ...
            match_str = re.search(pattern, text_corpus, re.IGNORECASE).group(0)
            prefix = text_corpus[max(0, text_corpus.find(match_str) - 15):text_corpus.find(match_str)]
